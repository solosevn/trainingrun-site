# CADENCE â TRS Site Manager (TRSitekeeper)
...
| Time (CST) | Activity |
|---|---|
| 4:00 AM | DDP Pipeline runs (`daily_runner.py` via cron) â Sitekeeper orchestrates |
| 4:30 AM | Post-DDP verification â check all 5 JSON files updated, spot-check scores |
| 6:00 AM | **Autonomous audit begins** â systematic site review (~1 hour) |
| 7:00 AM | Audit complete â daily findings report sent to David via Telegram |
| All day | **Reactive mode** â listening for David's Telegram messages |
...
| Saturday | Full site walkthrough (visitor perspective) | â |
| Sunday | Light check only â minimal processing | â |
...
| Weekly (Monday) | Week-in-review summary | Telegram â issues found, fixes made, suggestions pending |
| Monthly | Site health scorecard | Telegram â trends, recurring issues, improvement ideas |
...
```cron
# DDP Pipeline â daily at 4 AM CST
