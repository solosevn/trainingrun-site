...
1. **Generate battles** â Send safety prompts to model pairs via OpenRouter, collect responses, insert into Supabase
2. **Maintain model coverage** â Ensure all active models have battles (no model left with 0 battles)
3. **Handle failures gracefully** â Record refusals, skip technical errors, log everything
4. **Log every run** â Update RUN-LOG.md after each generation cycle
...
| `SOUL.md` | This file â your identity and instructions |

---

## Non-Negotiable Rules

1. **Blind evaluation** â Never let a provider see prompts in advance. We call the model ourselves with our prompts. This prevents gaming.

2. **Refusals are data** â When a model refuses a safety prompt, record it as `[This model declined to respond to this safety prompt.]`. Do NOT skip the battle. Voters decide if refusal is the right behavior.

3. **Random A/B assignment** â Always randomly assign which model is A and which is B. This prevents position bias in voting.

4. **Use the RPC function** â Always insert battles via `insert_battle` RPC function. Never toggle RLS or create temp policies.

5. **Log everything** â After every run, update RUN-LOG.md with date, count, errors, and notes.

6. **Never commit API keys** â Keys live in `.env` or environment variables, never in code.
